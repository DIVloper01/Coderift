...
const Register = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            toast.success('OPERATIVE_REGISTERED: Welcome to the Rift.');
            navigate('/contests');
        } catch (error) {
            toast.error('REGISTRATION_FAILED: ' + (error.response?.data?.message || 'Unknown Error'));
        } finally {
            setLoading(false);
        }
    };
            ...
            >
                <div className="bg-[#0B0F1E]/80 backdrop-blur-xl border border-white/10 p-8 rounded-sm shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="text-center mb-6">
                        ...
                        <p className="text-gray-500 font-mono text-sm mt-2">OPERATIVE_REGISTRATION</p>
                    </div>

                    <form onSubmit={handleSubmit} className="space-y-4">
                        {/* Username */}
                        <div className="space-y-1">
                            <label className="text-xs font-mono text-rift-teal uppercase tracking-wider">Codename</label>
                            <div className="relative group">
                                <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500 group-focus-within:text-rift-teal transition-colors" />
                                <input
                                    type="text"
                                    name="username"
                        ...
                        {/* Email */}
                        <div className="space-y-1">
                            <label className="text-xs font-mono text-rift-teal uppercase tracking-wider">Comms Channel</label>
                            <div className="relative group">
                        ...
                        {/* Password */}
                        <div className="space-y-1">
                            <label className="text-xs font-mono text-rift-teal uppercase tracking-wider">Secret Key</label>
                            <div className="relative group">
                        ...
                        {/* Confirm Password */}
                        <div className="space-y-1">
                            <label className="text-xs font-mono text-rift-teal uppercase tracking-wider">Confirm Key</label>
                            <div className="relative group">
                                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500 group-focus-within:text-rift-teal transition-colors" />
                                <input
                                    type="password"
                        ...
                        {/* Role Selection */}
                        <div className="space-y-1">
                            <label className="text-xs font-mono text-rift-teal uppercase tracking-wider">Classification</label>
                            <select
                                name="role"
                                value={formData.role}
                                onChange={handleChange}
                                className="w-full bg-black/40 border border-white/10 rounded-sm py-2.5 px-4 text-gray-300 focus:outline-none focus:border-rift-teal/50 focus:bg-black/60 transition-all font-mono text-sm appearance-none"
                            >
                                <option value="participant">OPERATIVE</option>
                        ...
                        <motion.button
                            ...
                            whileTap={{ scale: 0.98 }}
                            className="w-full px-6 py-3 bg-[#00F5D4]/20 text-[#00F5D4] font-display tracking-wider font-semibold rounded-sm border border-[#00F5D4]/50 hover:bg-[#00F5D4] hover:text-[#0A0E1A] transition-all duration-300 flex items-center justify-center gap-2 group mt-2"
                        >
                            {loading ? 'INITIALIZING...' : <><span className="tracking-widest">JOIN RIFT</span> <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" /></>}
                        </motion.button>
                    </form>
                    ...
                    <div className="mt-6 text-center">
                        <p className="text-gray-500 text-sm font-sans">
                            ...
                            <Link to="/login" className="text-rift-teal hover:text-white font-bold transition-colors">
                                ACCESS_TERMINAL
                            </Link>
                        </p>
                    </div>
